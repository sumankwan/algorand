...
class TestContract(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.new_creator_acct_priv_key, cls.new_creator_acct_addr = account.generate_account()
        cls.new_creator_acct_mnemonic = mnemonic.from_private_key(cls.new_creator_acct_priv_key)
        cls.new_creator_acct_addr = account.address_from_private_key(cls.new_creator_acct_priv_key)
        print("Generated creator new account: "+cls.new_creator_acct_addr)
        cls.new_user_acct_priv_key, cls.new_user_acct_addr = account.generate_account()
        cls.new_user_acct_mnemonic = mnemonic.from_private_key(cls.new_user_acct_priv_key)
        cls.new_user_acct_addr = account.address_from_private_key(cls.new_user_acct_priv_key)
        print("Generated user new account: "+cls.new_user_acct_addr)
        cls.app_index = 0
    
    #Methods for test cases must start with test
    def test_deploy_app(self):
        amt = 3000000
        fund_new_acct(TestContract.algod_client, TestContract.new_creator_acct_addr, amt, TestContract.funding_acct_mnemonic)    

        print("Funded {amt} to new account for the purpose of deploying contract".format(amt = amt))

        creator_private_key = get_private_key_from_mnemonic(TestContract.new_creator_acct_mnemonic)
        ...
        fund_new_acct(TestContract.algod_client, TestContract.new_user_acct_addr, amt, TestContract.funding_acct_mnemonic)
        
        user_private_key = get_private_key_from_mnemonic(TestContract.new_user_acct_mnemonic)

        opt_in_app(TestContract.algod_client, user_private_key, TestContract.app_index)

        wait_for_round(TestContract.algod_client, voteBegin)
        
        self.assertEqual(global_state['VoteBegin'], voteBegin)

        call_app(TestContract.algod_client, user_private_key, TestContract.app_index, [b"vote", b"choiceA"])

        print(
        "Local state:",
        read_local_state(
            TestContract.algod_client, account.address_from_private_key(user_private_key), TestContract.app_index
            ),
        )
        ...
        delete_app(TestContract.algod_client, creator_private_key, TestContract.app_index)

        clear_app(TestContract.algod_client, user_private_key, TestContract.app_index)
